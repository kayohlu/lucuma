defmodule HoldUp.AccountsTests.UserTest do
  ...
  describe "users" do
    ...
    test "get_user!/1 returns the user with given id" do
      user = user_fixture(%{company_id: company_fixture.id})

      assert Accounts.get_user!(user.id) == user
    end

    test "get_user/1 returns the user with given id when no user with the given id exists" do
    ...
    test "create_user/1 with valid data creates a user" do
      ...
      assert {:ok, %User{} = user} =
               Accounts.create_user(Map.put(@valid_attrs, :company_id, company.id))
