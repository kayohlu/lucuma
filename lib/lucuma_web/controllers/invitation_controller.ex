defmodule LucumaWeb.InvitationController do
  use LucumaWeb, :controller

  alias Lucuma.Accounts
  alias Lucuma.Accounts.User

  plug :put_layout, {LucumaWeb.LayoutView, :only_form} when action in [:show, :update]
  ...
  def create(conn, %{"user" => invitation_params}) do
    case Accounts.create_invited_user(
      ...
      {:ok, invited_user} ->
        LucumaWeb.Emails.Email.invitation_email(invited_user) |> LucumaWeb.Mailer.deliver_now()
  ...
  def update(conn, %{"id" => id, "invitation" => invited_user_params}) do
    ...
    case Accounts.accept_user_invite(invited_user, invited_user_params) do
      {:ok, user} ->
        ...
        |> LucumaWeb.Plugs.Authentication.sign_in_user(user)
