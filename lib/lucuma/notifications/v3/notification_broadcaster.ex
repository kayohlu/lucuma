defmodule Lucuma.Notifications.V3.NotificationBroadcaster do
  use GenStage
  require Logger
  ...
  """
  defp dispatch_events(queue, 0 = demand, events) do
    IO.puts("#{__MODULE__} dispatch_events to consumer since demand is gone down to zero")

    {:noreply, Enum.reverse(events), {queue, 0}}
  end

  @doc """
  This function is called when the demand value is > 0.
  Here we try to remove items from the queue until it's empty or the number
  of events removed from the queue reduces the demand value to 0
  """
  defp dispatch_events(queue, demand, events) do
    ...
    case :queue.out(queue) do
      {{:value, event}, queue} ->
        IO.puts("#{__MODULE__} dispatch_events populate events to send and reduce demand")
        IO.puts("#{__MODULE__} new demand #{demand - 1}")
